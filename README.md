...
| Farming `Invalid incentiveId` on exit/collect | Exit/collect was attempted when `tokenFarmedIn` and deposit `incentiveId` were already zero (not currently staked), or with stale key assumptions | Added hard execution gate in farm exit/collect plans (`BLOCKED` when not staked or simulation fails), plus `tx-verify` root-cause classification and exact remediation | Failing tx example: `0x205216d0ac6dfb3d1a00296769d4877948f2118e5c97f9355cb51ed92488eb0e` |
| Position manager low-gas reverts on LP management actions | Manual/malformed `collect`/`decreaseLiquidity`/`burn` calldata, including zero-max collect args and truncated ABI words | Extended `tx-verify` to decode these selectors, print exact calldata byte mismatches, and validate owner/approval/deadline/liquidity/tokens-owed preconditions with explicit remediation | Failing tx examples: `0x0dc240e5f09e9fdc96c0caf4e27dfc1889f0a5d19f4b3f98bc2930102a30c576`, `0xb74a6b19b8e8ca7021176ce5d84b872900e4b7edb4413a60d557e847e372b3e8` |
